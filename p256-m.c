                     ...
                     const m256_mod *mod)
{
    ...
    unsigned i;
    uint32_t a[9], carry_add, carry_sub, use_sub;
    ...
    /* a = a > m ? a - m : a */
    carry_add = a[8];  /* 0 or 1 since a < 2m, see HAC Note 14.37 */
    carry_sub = u256_sub(z, a, mod->m);
    use_sub = carry_add | (1 - carry_sub);     /* see m256_add() */
                        ...
                        const uint32_t s[8])
{
     ...
     */
    unsigned i;
    uint32_t s_odd[8], py_neg[8], py_use[8], rz[8], negate;
                      ...
                      const uint8_t *hash, size_t hlen)
{
    ...
    uint32_t r[8], s[8], e[8], u1[8], u2[8], px[8], py[8];
